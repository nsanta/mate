...
async function controller(node, response, func) {
	let current = node;
	while (current && !current.mxController) current = current.parentElement;
	if (current && current.mxController) current.mxController[func](response);
	else console.warn(`Controller not found for function "${func}"`);
}
const PRESENTERS = {
	"@inner": inner,
	"@outer": outer,
...
//#endregion
export { registerPresenter as i, hasPresenter as n, present as r, getPresenter as t };
